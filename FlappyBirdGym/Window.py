...
class Window(tk.Frame):
    ...
    def updatePlots(self, v, a, reward, layerActivations):
        ...
        collectedRewards_plt.plot(self.steps,self.collectedRewards, label="Reward")
        ...
        collectedRewards_part = self.collectedRewards[max(0, self.step_cnt - 50):self.step_cnt]
        mean_collectedRewards_part = np.mean(collectedRewards_part)
        collectedRewards_plt.axhline(mean_collectedRewards_part, color='r', linestyle="--", label="Mean")
        collectedRewards_plt.legend(loc='lower right')
        #
