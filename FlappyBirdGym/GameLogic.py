...
class GameLogic:
    ...
    def init_logic(self):
        ...
        self.bird_pos_x0 = 15
        self.bird_pos_x1 = 30
        self.bird_pos_y0 = int(self.height / 2)
        self.bird_pos_y1 = int(self.height / 2) + 15

        # Draw it if window is used
        if self.window_mode != WindowMode.NO_WINDOW:
            self.window.canvas.create_rectangle(
                self.bird_pos_x0, self.bird_pos_y0, self.bird_pos_x1, self.bird_pos_y1,
                fill="red",
                outline='red',
                tags=('bird')
            )
    ...
    def next_game_step(self, action):
        ...
        self.bird_pos_y0 += delta_y
        self.bird_pos_y1 += delta_y
        ...
        if self.height <= self.bird_pos_y1 or self.bird_pos_y0 <= 0:
            killed = True
            reward = -1
            ...
            for column in self.columns:
                ...
                if column.get_pos_x() + column.column_width <= self.bird_pos_x0 and not column.was_bird_flown_over():
                    column.set_bird_flown_over(True)
                    self.first_column = self.columns[1]

                bird_pos = self.bird_pos_x0, self.bird_pos_y0, self.bird_pos_x1, self.bird_pos_y1
    ...
    def get_state(self):
        ...
        return np.array([self.bird_pos_x0, self.bird_pos_y0,
